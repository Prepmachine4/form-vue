    ...
    <el-dialog top="5vh" title="审核" v-model="dialogVisible" width="80vw" >
      ...
      <span style="display: flex;width: 100%;justify-content: center">
        ...
        <el-form-item required label="签名">
          <vue-esign ref="sign" :isClearBgColor="false" bgColor="#eceef1" :width="300" :height="150" :isCrop="false" :lineWidth="6" lineColor="#000000"  />
        </el-form-item>
      </el-form>
      </span>
      <br>
...
const onsubmit = (result) => {
  ...
  sign.value.generate().then(sign => {
    console.log(sign)
    postAudit(selectRow.value._id,{user_id:store.state.userInfo._id,index:selectRow.value.audit_user_index,opinion:opinion.value,sign,result})
        .then(res=>{
