    ...
    <el-container>
      <el-aside  width="200px">
        <el-menu @select="selectMenu" style="height: 100vh;" active-text-color="#ffd04b" background-color="#545c64" class="el-menu-vertical-demo" default-active="0" text-color="#fff">
          ...
          <template v-if="user_info.enterprise_id">
            <el-sub-menu  index="3">
              <template #title>
            ...
            <el-menu-item index="5">
              ...
              <el-badge  style="line-height: 30px" :hidden="hiddenAuditBadge" :value="auditBadge" >
                <span>审核管理</span>
              </el-badge>
            </el-menu-item>
          </template>
...
import {listNeedAudit} from "@/api/audit";
import {ElNotification} from "element-plus";
const router = useRouter()
const route = useRoute()
const store = useStore()
let user_info = computed(() => store.state.userInfo)
const auditBadge=ref(0)
const hiddenAuditBadge=ref(true)
onMounted(()=>{
//  是否提示审批
  listNeedAudit().then(res=>{
    if(res.data.length!==0) {
      auditBadge.value=res.data.length
      hiddenAuditBadge.value=false
      ElNotification.info(`您有${auditBadge.value}条业务表单等待您审批`)
    }
  })
})
